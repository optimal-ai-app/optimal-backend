...
public class TaskCreatorAgent extends BaseAgent {
    ...
    private boolean shouldInitializeMilestoneTool(List<Message> context) {
        for (Message msg : context) {
            ...
            if (content != null && content.contains("milestones that need to be created")) {
                System.out.println("[TaskCreatorAgent] Detected milestone list in context");
                return true;
            }
        }
        return false;
    ...
    private void initializeMilestoneToolFromContext(List<Message> context) {
        ...
        if (!milestones.isEmpty()) {
            ...
            System.out.println("✓ TaskCreatorAgent: Initialized MilestoneQueueTool with " + milestones.size() + " milestones");
        } else {
            System.out.println("⚠ TaskCreatorAgent: No milestones found in context");
        }
    }
    ...
    private Queue<MilestoneQueueTool.MilestoneData> parseMilestoneList(List<Message> context) {
        ...
        for (Message msg : context) {
            ...
            System.out.println("[TaskCreatorAgent] Parsing milestone list from: " + content);
            
            // Extract goal name
            String goalName = extractGoalName(content);
            System.out.println("[TaskCreatorAgent] Goal name: " + goalName);
            
            // Extract milestone strings
            List<String> milestoneStrings = extractMilestoneStrings(content);
            System.out.println("[TaskCreatorAgent] Found " + milestoneStrings.size() + " milestones");
            ...
            for (String ms : milestoneStrings) {
                ...
                if (data != null) {
                    milestones.add(data);
                    System.out.println("[TaskCreatorAgent] Parsed milestone: " + data);
                }
            }
