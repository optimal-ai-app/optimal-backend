...
public class LlmDateSuggestionTool {
    ...
    public String SuggestDate(@P("dateInput") String dateInput) {
        ...
        // Check if input is a relative date pattern - if so, calculate and cross-check
        // with GetFutureDateTool
        ...
        if (calculatedDate != null) {
            ...
            if (!calculatedDate.toString().equals(crossCheckDate)) {
                System.out.println(
                        "Warning: Direct calculation differs from GetFutureDateTool, using GetFutureDateTool result");
        ...
        // For non-relative dates (like "Nov 23", "December 15", "next Monday", etc.),
        // use LLM
        ...
        // The LLM should return just YYYY-MM-DD, but we'll try to extract it if there's
        // extra text
     ...
     * Calculate relative dates from patterns like "in 3 weeks", "in 2 months", etc.
     * Returns null if the input doesn't match a relative date pattern.
     * Handles various formats including "in X days/weeks/months/years", "X
    ...
    private String extractDateFromResponse(String response) {
        ...
        // If no pattern found, return the trimmed response as-is (might be just the
        // date)
